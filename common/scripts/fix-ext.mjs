		...
		if (possiblePath.length) {
			for (let i = 0; i < possiblePath.length; i++) {
				...
				if (fs.existsSync(entry)) {
					const resolved = path.relative(root, entry.replace(/\.tsx?$/, '.js'));
...
const defaultSourceFileFilter = function (sourceFilePath) {
	return /\.(jsx?|tsx?)$/.test(sourceFilePath) && !/node_modules/.test(sourceFilePath);
};
...
if (DEBUG) {
	replace('/Users/ivan.mjartan/repositories/gooddata/gooddata-ui-sdk/libs/sdk-ui-dashboard/src/index.ts', './out.ts', defaultOptions);
} else {
	await run(directory, defaultOptions);
}
