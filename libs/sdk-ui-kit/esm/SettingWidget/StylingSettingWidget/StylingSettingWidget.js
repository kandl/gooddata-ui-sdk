...
const StylingSettingWidgetCore = (props) => {
    const { title, defaultItem, customItems, itemToColorPreview, emptyMessage, selectedItemRef, isLoading, titleTooltip, footerHelpLink, footerHelpTitle, footerMobileMessage, shouldDisableCancelButton, shouldDisableApplyButton, onApply, onCancel, onListActionClick, onItemEdit, onItemDelete, className, onHelpClick, onItemSelect = noop, onItemMenuToggle, } = props;
    const intl = useIntl();
    const isMobileDevice = useMediaQuery("mobileDevice");
    const initiallySelectedItemRef = selectedItemRef || null;
    const [currentItemRef, setCurrentItemRef] = useState(initiallySelectedItemRef);
    useEffect(() => {
    ...
    const onItemClick = useCallback((ref) => {
        onItemSelect(ref);
        setCurrentItemRef(ref);
    }, [onItemSelect]);
    const isApplyButtonDisabled = useMemo(() => areObjRefsEqual(currentItemRef, selectedItemRef), [currentItemRef, selectedItemRef]);
 ...
 * @internal
 */
export const StylingSettingWidget = (props) => {
    return (React.createElement(IntlWrapper, { locale: props.locale },
        React.createElement(StylingSettingWidgetCore, Object.assign({}, props))));
};
