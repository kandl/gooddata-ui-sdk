...
import { ColumnChartViewByDateAndPop, ColumnChartWithSingleMeasureViewByAndStackBy, ColumnChartWithTwoMeasuresAndViewBy, } from "./base.js";
import { replaceMappingPredicates } from "../_infra/insightConverters.js";
import { measureLocalId } from "@gooddata/sdk-model";
import { ReferenceData, ReferenceMd } from "@gooddata/reference-workspace";
import { ScenarioGroupNames } from "../_infra/groupNames.js";
const colorsAndPalette = scenariosFor("ColumnChart", ColumnChart)
    .withGroupNames(...ScenarioGroupNames.Coloring)
    .withVisualTestConfig({ groupUnder: "coloring" })
    .withDefaultTags("vis-config-only", "mock-no-scenario-meta")
    .addScenarios("coloring", ColumnChartWithTwoMeasuresAndViewBy, coloringCustomizer);
const colorAssignment = scenariosFor("ColumnChart", ColumnChart)
    .withGroupNames(...ScenarioGroupNames.Coloring)
    .withDefaultTags("vis-config-only", "mock-no-scenario-meta")
...
}, (m) => m.withInsightConverter(replaceMappingPredicates(measureLocalId(ReferenceMd.Amount), measureLocalId(ReferenceMd.Won))))
...
}, (m) => m.withInsightConverter(replaceMappingPredicates(ReferenceData.Region.EastCoast.uri, ReferenceData.Region.WestCoast.uri)));
