...
const colorsAndPalette = scenariosFor("BubbleChart", BubbleChart)
    .withGroupNames(...ScenarioGroupNames.Coloring)
    .withVisualTestConfig({ groupUnder: "coloring" })
    .withDefaultTags("vis-config-only", "mock-no-scenario-meta")
    .addScenarios("coloring", BubbleChartWithAllMeasuresAndAttribute, coloringCustomizer);
const colorAssignment = scenariosFor("BubbleChart", BubbleChart)
    .withGroupNames(...ScenarioGroupNames.Coloring)
    .withDefaultTags("vis-config-only", "mock-no-scenario-meta")
...
}, (m) => m.withInsightConverter(replaceMappingPredicates(ReferenceData.ProductName.Explorer.uri, ReferenceData.ProductName.WonderKid.uri)));
