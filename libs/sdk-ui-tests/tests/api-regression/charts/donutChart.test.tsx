...
import donutChartScenarios from "../../../scenarios/charts/donutChart/index.js";
...
vi.mock("@gooddata/sdk-ui-charts/internal-tests/CoreDonutChart", async () => {
    const Original = await vi.importActual<any>("@gooddata/sdk-ui-charts/internal-tests/CoreDonutChart");
    const { extractProps: originalExtractProps, wrap } = withPropsExtractor();
    extractProps = originalExtractProps;

    return {
        ...Original,
        CoreDonutChart: wrap(Original.CoreDonutChart),
    };
});

describe(Chart, () => {
    const Scenarios: Array<ScenarioAndDescription<IDonutChartProps>> = flatMap(donutChartScenarios, (group) =>
