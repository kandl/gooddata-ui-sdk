...
describe("DashboardLayoutContentRenderer", () => {
        ...
        const dashboardLayoutFacade = FluidLayoutFacade.for(
            dashboardLayoutMock([
                dashboardRowMock([
                    [
                        dashboardWidgetMock("kpi1", "kpi"),
                        { xl: { widthAsGridColumnsCount: 4, heightAsRatio: 150 } },
                    ],
                ]),
            ]),
        );
    ...
    it("should propagate minHeight, when heightAsRatio is missing", () => {
        const dashboardLayoutFacade = FluidLayoutFacade.for(
            dashboardLayoutMock([dashboardRowMock([[dashboardWidgetMock("kpi1", "kpi")]])]),
        );
        const minHeight = 100;
    ...
    it("should not propagate minHeight, when heightAsRatio is defined", () => {
        const dashboardLayoutFacade = FluidLayoutFacade.for(
            dashboardLayoutMock([
                dashboardRowMock([
                    [
        ...
        expect(wrapper.find("div")).not.toHaveStyle("minHeight", minHeight);
    });

    it("should propagate height, when heightAsRatio is defined", () => {
        const dashboardLayoutFacade = FluidLayoutFacade.for(
            dashboardLayoutMock([
                dashboardRowMock([
                    [
    ...
    it("should not propagate height, when heightAsRatio is not defined", () => {
        const dashboardLayoutFacade = FluidLayoutFacade.for(
            dashboardLayoutMock([dashboardRowMock([[dashboardWidgetMock("kpi1", "kpi")]])]),
        );
        const height = 100;
        ...
        expect(wrapper.find("div")).not.toHaveStyle("height", height);
    });
});
