...
import { getAllAttributeItems, getAllMeasures, getPreferredBucketItems, limitNumberOfMeasuresInBuckets, removeAllArithmeticMeasuresFromDerived, removeAllDerivedMeasures, sanitizeFilters, } from "../../../utils/bucketHelper.js";
...
export class PluggableBubbleChart extends PluggableBaseChart {
    ...
    getExtendedReferencePoint(referencePoint) {
        ...
        let newReferencePoint = Object.assign(Object.assign({}, clonedReferencePoint), { uiConfig: cloneDeep(DEFAULT_BUBBLE_CHART_CONFIG) });
        ...
        const secondaryMeasuresBucketItems = getPreferredBucketItems(buckets, [BucketNames.SECONDARY_MEASURES], [METRIC]);
        const tertiaryMeasuresBucketItems = getPreferredBucketItems(buckets, [BucketNames.TERTIARY_MEASURES], [METRIC]);
        ...
        const measures = measuresBucketItems.length > 0
            ? measuresBucketItems.slice(0, 1)
            : allMeasures.filter((measure) => !includes(secondaryAndTertiaryItems, measure)).slice(0, 1);
        const secondaryMeasures = secondaryMeasuresBucketItems.length > 0
            ? secondaryMeasuresBucketItems.slice(0, 1)
            : allMeasures
                .filter((measure) => !includes([...measures, ...tertiaryMeasuresBucketItems.slice(0, 1)], measure))
                .slice(0, 1);
        const tertiaryMeasures = tertiaryMeasuresBucketItems.length > 0
            ? tertiaryMeasuresBucketItems.slice(0, 1)
            : allMeasures
