            ...
            (widget, dashboardFilters, crossFilteringFiltersLocalIdentifiers) => {
                ...
                const filtersWithoutCrossFilteringFilters = dashboardFilters.filter(f => {
                    if (isDashboardAttributeFilter(f)) {
                        return !crossFilteringFiltersLocalIdentifiers?.includes(f.attributeFilter.localIdentifier!);
                    }

                    return true;
                });
                return filterContextItemsToDashboardFiltersByWidget(filtersWithoutCrossFilteringFilters, widget);
            },
        );
    });
