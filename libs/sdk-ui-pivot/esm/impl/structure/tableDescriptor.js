import { isScopeCol, isSeriesCol, isSliceCol, isRootCol, agColId, isEmptyScopeCol, isSliceMeasureCol, isMixedHeadersCol, } from "./tableDescriptorTypes.js";
...
import { createSortIndicators } from "./tableDescriptorSorting.js";
...
export class TableDescriptor {
    constructor(dv, headers, colDefs) {
        this.dv = dv;
    ...
    static _getMeasureGroupDimensionIndex(dv) {
     ...
     */
    static for(dv, emptyHeaderTitle, config, intl) {
        const isTransposed = TableDescriptor.isTransposed(dv);
        const { headers, colDefs } = createHeadersAndColDefs(dv, emptyHeaderTitle, isTransposed, config, intl);
     ...
     */
    matchAttributeWidthItem(columnWidthItem) {
        const matcher = attributeDescriptorLocalIdMatch(columnWidthItem.attributeColumnWidthItem.attributeIdentifier);
     ...
     */
    matchMeasureWidthItem(measureWidthItem) {
     ...
     * @param measureWidthItem - item to match
     */
    matchSliceMeasureWidthItem(sliceMeasureWidthItem) {
        return searchForTransposedLocatorMatch(this.headers.sliceMeasureCols, sliceMeasureWidthItem.sliceMeasureColumnWidthItem.locators);
    }
    /**
     ...
     * @param measureWidthItem - item to match
     */
    matchMixedValuesWidthItem(mixedValuesWidhItem) {
        return searchForTransposedLocatorMatch(this.headers.mixedValuesCols, mixedValuesWidhItem.mixedValuesColumnWidthItem.locators);
    }
    /**
     ...
     * are returned.
     */
    getSliceColsUpToIncludingCol(col) {
     ...
     */
    createSortItems(columns, originalSorts = []) {
        if (columns.length === 0) {
            return [];
        }
        const sortIndicators = createSortIndicators(columns);
        return sortIndicators.map((sortModelItem) => {
     ...
     */
    updateEffectiveTotals(dv) {
        const idToDescriptor = keyBy(dv.meta().attributeDescriptors(), (desc) => desc.attributeHeader.localIdentifier);
