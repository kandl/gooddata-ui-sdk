import { getTotalInfo } from "@gooddata/sdk-ui";
import { isAttributeDescriptor, isResultTotalHeader, MeasureGroupIdentifier, } from "@gooddata/sdk-model";
...
function groupColumns(bottomColumns, scopingAttributes, limitLevels) {
    ...
    const columnGroupLevels = range(levels).map((_) => ({
        pkToGroup: {},
        groups: [],
    }));
    const allColumns = [...bottomColumns, root];
    let groupId = 0;
    for (const leaf of bottomColumns) {
        ...
        for (let level = 0; level < levels; level++) {
            ...
            const currentId = isResultTotalHeader(resultHeader)
                ? (_a = resultHeader === null || resultHeader === void 0 ? void 0 : resultHeader.totalHeaderItem) === null || _a === void 0 ? void 0 : _a.type
                : (_b = resultHeader === null || resultHeader === void 0 ? void 0 : resultHeader.attributeHeaderItem) === null || _b === void 0 ? void 0 : _b.uri;
            pathToGroup += currentId;
            const groupLevel = columnGroupLevels[level];
            let currentGroup = groupLevel.pkToGroup[pathToGroup];
            const { isTotal, isSubtotal } = getTotalInfo(attributeHeaders);
            if (!currentGroup) {
                const fullIndexPathToHere = parentGroup
 ...
 */
function createColumnDescriptorsWhenNoMeasuresInColumns(dv, isTransposed) {
    var _a, _b;
    const descriptors = dv.meta().attributeDescriptorsForDim(1);
    const headers = dv.meta().attributeHeadersForDim(1);
    const numberOfColumns = (_b = (_a = headers[0]) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0;
    ...
    for (let colIdx = 0; colIdx < numberOfColumns; colIdx++) {
        ...
        const { isTotal, isSubtotal } = getTotalInfo(attributeHeaders);
        bottom.push(Object.assign({ type: "scopeCol", id: `cg_${colIdx}`, children: [], fullIndexPathToHere: [colIdx], header: headers[numberOfAttributes - 1][colIdx], headersToHere: headers.slice(0, numberOfAttributes - 1).map((attrHeaders) => attrHeaders[colIdx]), attributeDescriptor: descriptors[numberOfAttributes - 1], descriptorsToHere: descriptors.slice(0, numberOfAttributes - 1), isTotal,
            isSubtotal }, (isTransposed
            ? {
                measureDescriptors: seriesDescriptor.map((serieDescriptor) => serieDescriptor.measureDescriptor),
            }
            : {})));
    }
...
function createColumnDescriptorsFromDataSeries(dv) {
    ...
    const scopingAttributes = (_a = dv.data().series().scopingAttributes) !== null && _a !== void 0 ? _a : [];
...
function createRowDescriptor(attributeDescriptor, index) {
    var _a;
    return {
        type: "sliceCol",
        id: `r_${index}`,
...
function createColumnDescriptors(dv, isTransposed) {
...
function getMeasureGroupDimensionIndex(dv) {
    return dv.definition.dimensions.findIndex((dimension) => dimension.itemIdentifiers.includes(MeasureGroupIdentifier));
}
function createMeasureColumnDescriptors(dv, rows) {
    if (getMeasureGroupDimensionIndex(dv) !== 0) {
        return [];
    }
...
function createMixedHeadersColumnDescriptors() {
    const idx = 0;
...
function createMixedValuesColumnDescriptors(dv) {
    return dv
        .data()
        .slices()
        .toArray()
...
function createMeasureValuesColumnDescriptors(dv) {
    const idx = 0;
    // always just one column with mixed attribute and measure headers
    return [
        {
...
function createTableHeaders(dv, isTransposed, config) {
    var _a;
    const idToDescriptor = {};
    if ((config === null || config === void 0 ? void 0 : config.columnHeadersPosition) === "left" && isTransposed) {
        ...
        const scopingAttributes = (_a = dv.data().slices().descriptors) !== null && _a !== void 0 ? _a : [];
        return {
            sliceCols: [],
            sliceMeasureCols: [],
            rootDataCols: [],
        ...
        const { rootColumns, leafColumns, allColumns, groupingAttributes } = createColumnDescriptors(dv, isTransposed);
 ...
 * @internal
 */
export function createHeadersAndColDefs(dv, emptyHeaderTitle, isTransposed, config, intl) {
    const headers = createTableHeaders(dv, isTransposed, config);
    const colDefs = createColDefsFromTableDescriptor(headers, dv.meta().effectiveSortItems(), emptyHeaderTitle, isTransposed, config, intl);
