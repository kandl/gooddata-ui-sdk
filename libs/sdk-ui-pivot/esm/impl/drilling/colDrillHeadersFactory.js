...
import { isScopeCol, isSeriesCol, isSliceCol, isMixedValuesCol, } from "../structure/tableDescriptorTypes.js";
...
export function createDataColLeafHeaders(col) {
    ...
    if (col.seriesDescriptor.attributeDescriptors) {
        col.seriesDescriptor.attributeDescriptors.forEach((attributeDescriptor, index) => {
 ...
 */
export function createScopeColWithMetricHeaders(col, includeColGroupHeaders, row) {
    const mappingHeaders = includeColGroupHeaders ? createDataColGroupHeaders(col) : [];
    if (row === null || row === void 0 ? void 0 : row.measureDescriptor) {
        mappingHeaders.push(row.measureDescriptor);
    }
    return mappingHeaders;
}
export function createMixedValuesColHeaders(col, row) {
    const mappingHeaders = [];
...
function createDataColGroupHeaders(col) {
    const mappingHeaders = [];
 ...
 * @param row - row
 */
export function createDrillHeaders(col, row, columnHeadersPosition, isTransposed) {
    if (isSeriesCol(col)) {
