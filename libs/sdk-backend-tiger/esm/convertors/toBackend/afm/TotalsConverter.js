...
import { bucketsFind, bucketItemLocalId, bucketAttributes, bucketTotals, MeasureGroupIdentifier, } from "@gooddata/sdk-model";
import flatMap from "lodash/flatMap.js";
import { TotalFunctionEnum } from "@gooddata/api-client-tiger";
import { dimensionLocalIdentifier } from "./DimensionsConverter.js";
const TOTAL_ORDER = ["SUM", "MAX", "MIN", "AVG", "MED"];
const ATTRIBUTE = "attribute";
const COLUMNS = "columns";
function getMeasureOrder(total, measures) {
    return measures.findIndex((m) => m.measure.localIdentifier === total.metric);
}
function getFunctionOrder(total) {
    return TOTAL_ORDER.findIndex((item) => item === total.function);
}
function enrichTotalWithMeasureIndex(total, measures) {
    return {
...
export function convertTotals(def) {
    ...
    const allTotalDimensions = dimensions.map((dim, dimIdx) => {
        return {
    ...
    const totals = flatMap(dimensions, (dim, dimIdx) => {
        var _a;
        const mappedTotals = (_a = dim.totals) === null || _a === void 0 ? void 0 : _a.map((total) => {
            const tigerTotal = {
                localIdentifier: totalLocalIdentifier(total, dimIdx),
                function: convertTotalType(total.type),
                metric: total.measureIdentifier,
                totalDimensions: convertTotalDimensions(total, dim, dimensionLocalIdentifier(dimIdx), allTotalDimensions),
            };
            return tigerTotal;
        });
        ...
        const totalsWithOrders = mappedTotals === null || mappedTotals === void 0 ? void 0 : mappedTotals.map((total) => enrichTotalWithMeasureIndex(total, def.measures));
        return totalsWithOrders === null || totalsWithOrders === void 0 ? void 0 : totalsWithOrders.sort((total1, total2) => {
    ...
    const matchingMetricGroupDimension = dimensions.findIndex((dimension) => { var _a; return (_a = dimension.itemIdentifiers) === null || _a === void 0 ? void 0 : _a.includes(MeasureGroupIdentifier); });
    const addMeasureGroupDim0 = matchingMetricGroupDimension === 0 ? [MeasureGroupIdentifier] : [];
    const addMeasureGroupDim1 = matchingMetricGroupDimension === 1 ? [MeasureGroupIdentifier] : [];
    ...
    if (hasRowOrColumnTotals(attributeBucket, columnBucket)) {
        ...
        (_a = attributeBucket.totals) === null || _a === void 0 ? void 0 : _a.forEach((attributeTotal, index) => {
            var _a;
            ...
            (_a = columnBucket.totals) === null || _a === void 0 ? void 0 : _a.forEach((columnTotal, columnIndex) => {
                ...
                if (attributeAndColumnHasSameTotal(attributeMeasureId, attributeType, columnMeasureId, columnType)) {
                    ...
                    if (attributeTotal.attributeIdentifier === attributeIdentifiers[0] &&
                    ...
                    if (attributeTotal.attributeIdentifier !== attributeIdentifiers[0] &&
                        columnTotal.attributeIdentifier !== columnIdentifiers[0]) {
                        attributeDimensionIndex = attributeIdentifiers.findIndex((aI) => aI === attributeTotal.attributeIdentifier);
                        columnDimensionIndex = columnIdentifiers.findIndex((cI) => cI === columnTotal.attributeIdentifier);
                    ...
                    if (attributeTotal.attributeIdentifier !== attributeIdentifiers[0] &&
                        ...
                        attributeSubtotalDimensionIndex = attributeIdentifiers.findIndex((aI) => aI === attributeTotal.attributeIdentifier);
                    }
                    // Check for columns subtotals within rows grand totals
                    if (attributeTotal.attributeIdentifier === attributeIdentifiers[0] &&
                        columnTotal.attributeIdentifier !== columnIdentifiers[0]) {
                        hasColumnSubtotalAndRowGrandTotal = true;
                        columnSubtotalDimensionIndex = columnIdentifiers.findIndex((cI) => cI === columnTotal.attributeIdentifier);
                    }
                }
                /**
 ...
 */
function convertTotalDimensions(total, dimension, dimensionIdentifier, allTotalDimensions) {
    ...
    if (dimension.itemIdentifiers.includes(MeasureGroupIdentifier) &&
...
function getAttributeBucketIdentifiers(attribute) {
 ...
 */
function hasRowOrColumnTotals(attributeBucket, columnBucket) {
...
function attributeAndColumnHasSameTotal(attributeMeasureId, attributeType, columnMeasureId, columnType) {
