...
const getDashboardConfig = (config = {}, allowUnfinishedFeatures) => {
    if (allowUnfinishedFeatures === "alwaysAllow" || allowUnfinishedFeatures === "staticOnly") {
        return Object.assign(Object.assign({}, config), { allowUnfinishedFeatures: true });
    }
    return Object.assign(Object.assign({}, config), { allowUnfinishedFeatures: false });
};
/**
...
export function useDashboardLoader(options) {
    ...
    const { dashboard, filterContextRef, config, permissions, clientWorkspace, loadingMode, extraPlugins, adaptiveLoadOptions, allowUnfinishedFeatures = "alwaysPrevent", } = options;
    const dashboardRef = typeof dashboard === "string" ? idRef(dashboard) : isDashboard(dashboard) ? dashboard.ref : dashboard;
    useEffect(() => {
        return () => {
    ...
    useCancelablePromise({
        ...
        onSuccess: (result) => {
            ...
            // eslint-disable-next-line no-console
            console.log(`Dashboard engine ${result.engine.version} initialized with plugins`, result.plugins.map((plugin) => `${plugin.displayName}/${plugin.version}`).join(", "));
            setLoadStatus({
                status: "success",
                result,
                error: undefined,
