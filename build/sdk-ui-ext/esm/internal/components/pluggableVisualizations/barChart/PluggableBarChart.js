...
export class PluggableBarChart extends PluggableColumnBarCharts {
    ...
    renderConfigurationPanel(insight) {
        ...
        if (configPanelElement) {
            this.renderFun(React.createElement(BarChartConfigurationPanel, { locale: this.locale, colors: this.colors, references: this.references, properties: this.visualizationProperties, propertiesMeta: this.propertiesMeta, insight: insight, pushData: this.handlePushData, type: this.type, isError: this.getIsError(), isLoading: this.isLoading, featureFlags: this.featureFlags, axis: this.axis }), configPanelElement);
        }
    }
    getDefaultAndAvailableSort(referencePoint, canSortStackTotalValue) {
            ...
            if (measures.length >= 2 && !canSortStackTotalValue) {
                return {
                    ...
                    availableSorts: [
                        ...
                        newAvailableSortsGroup(viewBy[1].localIdentifier, measures.map((m) => m.localIdentifier)),
                    ],
                };
            }
            return {
                ...
                availableSorts: [
                    newAvailableSortsGroup(viewBy[0].localIdentifier),
                    newAvailableSortsGroup(viewBy[1].localIdentifier, isEmpty(stackBy) ? measures.map((m) => m.localIdentifier) : [], true, isStacked || measures.length > 1),
                ],
            };
        }
    ...
    getSortConfig(referencePoint) {
        ...
        const { defaultSort, availableSorts } = this.getDefaultAndAvailableSort(referencePoint, canSortStackTotalValue(buckets, properties));
        const { disabled, disabledExplanation } = this.isSortDisabled(referencePoint);
        return Promise.resolve(Object.assign({ supported: true, disabled, appliedSort: super.reuseCurrentSort(previousAvailableSorts, properties, availableSorts, defaultSort), defaultSort, availableSorts: availableSorts }, (disabledExplanation && { disabledExplanation })));
    }
}
