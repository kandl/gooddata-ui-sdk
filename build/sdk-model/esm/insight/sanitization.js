...
import { insightAttributes, insightBuckets, insightSetBuckets, insightSorts, } from "./index.js";
import { bucketAttributeIndex, bucketSetTotals, bucketTotals } from "../execution/buckets/index.js";
import { isAttributeSort, isMeasureSort, sortEntityIds } from "../execution/base/sort.js";
...
function removeInvalidTotalsFromInsight(insight) {
    ...
    const sanitizedBuckets = insightBuckets(insight).map((bucket) => {
        const sanitizedTotals = sanitizeBucketTotals(bucket, sortItems).filter((total) => attributeIdentifiers.includes(total.attributeIdentifier));
...
export function sanitizeBucketTotals(bucket, sortItems, totals) {
    const originalTotals = totals !== null && totals !== void 0 ? totals : bucketTotals(bucket);
