...
import { DataRecorderError, isDataRecorderError } from "./base/types.js";
...
program
    ...
    .option("--replace-project-id <id>", `Replace projectId with this value when writing out recordings. By default not specified and projectId will stay in recordings as-is.`)
...
async function promptForMissingConfig(config) {
    ...
    try {
        ...
        await getOrInitBackend(username, password, hostname || DEFAULT_HOSTNAME, backend || DEFAULT_BACKEND).authenticate();
        ...
        if (((_a = err.message) === null || _a === void 0 ? void 0 : _a.search(/.*(certificate|self-signed).*/)) > -1) {
            logError("It seems that this error is due to invalid certificates used on the server. " +
    ...
    return Object.assign(Object.assign({}, config), { projectId,
        username,
        password });
}
async function captureRecordings(recordings, backend, config) {
    const executionSpinner = ora();
    let successes = 0;
    executionSpinner.start("Running and capturing executions");
    const onCaptured = (_, err) => {
        if (err) {
    ...
    const results = await pmap(recordings, (rec) => {
        var _a;
        return rec
            .makeRecording(backend, config.projectId, (_a = config.replaceProjectId) !== null && _a !== void 0 ? _a : undefined)
            .then((_) => {
            onCaptured(rec);
            return rec;
        })
            .catch((err) => {
            onCaptured(rec, `An error '${err}' has occurred while obtaining data for recording in ${rec.directory}; it is highly likely that the recording definition is semantically incorrect.`);
            return rec;
        });
    }, { concurrency: 4 });
...
async function run() {
    ...
    logInfo(`Discovered ${recordings.length} recordings; out of these ${incompleteRecordings.length} are missing recorded data.`);
    ...
    if (incompleteRecordings.length) {
         ...
         * run.
         */
        const backend = getOrInitBackend(fullConfig.username, fullConfig.password, options.hostname || DEFAULT_HOSTNAME, options.backend || DEFAULT_BACKEND);
