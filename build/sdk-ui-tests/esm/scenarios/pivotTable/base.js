...
import { PivotTable, newWidthForAttributeColumn, newWidthForAllColumnsForMeasure, } from "@gooddata/sdk-ui-pivot";
import { ScenarioGroupNames } from "../charts/_infra/groupNames.js";
import { requestPages } from "@gooddata/mock-handling";
import { modifyAttribute, newAbsoluteDateFilter } from "@gooddata/sdk-model";
export function getCommonPivotTableSizingConfig(attributesUsed = []) {
    return {
        columnSizing: {
            columnWidths: [
...
const modifiedCreatedYear = modifyAttribute(ReferenceMd.DateDatasets.Created.Year.Default, (m) => m.localId("created.test"));
export const PivotTableWithTwoSameDate = {
    measures: [],
    rows: [ReferenceMd.DateDatasets.Created.Year.Default, modifiedCreatedYear],
    columns: [],
};
const modifiedProductName = modifyAttribute(ReferenceMd.Product.Name, (m) => m.localId("product.name.test"));
export const PivotTableWithRepeatingRowAttributes = {
    measures: [ReferenceMd.Amount],
    rows: [ReferenceMd.Product.Name, ReferenceMd.Region, modifiedProductName],
    columns: [],
};
export default scenariosFor("PivotTable", PivotTable)
    ...
    .addScenario("single measure with two row and two column attributes", PivotTableWithSingleMeasureAndTwoRowsAndCols)
    ...
    .addScenario("two measures with two row and two column attributes", PivotTableWithTwoMeasuresAndTwoRowsAndCols, (m) => m.withCustomDataCapture({ windows: requestPages([0, 0], [22, 1000], 1) }))
    .addScenario("two measures with three rows and two column attributes", PivotTableWithTwoMeasuresAndThreeRowsAndTwoCols)
    ...
    .addScenario("one measure and repeating row attributes on different positions", PivotTableWithRepeatingRowAttributes)
    .addScenario("with date filter", {
