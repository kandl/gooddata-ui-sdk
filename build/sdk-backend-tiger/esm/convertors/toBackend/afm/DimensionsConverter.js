...
import { SortKeyAttributeAttributeDirectionEnum, SortKeyAttributeAttributeSortTypeEnum, } from "@gooddata/api-client-tiger";
import { isAttributeAreaSort, isAttributeLocator, isMeasureLocator, isAttributeSort, MeasureGroupIdentifier, } from "@gooddata/sdk-model";
import isEmpty from "lodash/isEmpty.js";
import findIndex from "lodash/findIndex.js";
function merge(dims, sorting) {
    return dims.map((dim, idx) => {
        if (!isEmpty(sorting[idx])) {
            return Object.assign(Object.assign({}, dim), { sorting: sorting[idx] });
        }
        return dim;
    });
}
 ...
 */
function convertAttributeSortType(sortItem) {
...
function extractItemValueFromElement(elementUri) {
    ...
    const parsedUri = elementUri === null || elementUri === void 0 ? void 0 : elementUri.match(/obj\/([^/]*)(\/elements\?id=)?(.*)$/);
    if (parsedUri === null || parsedUri === void 0 ? void 0 : parsedUri[3]) {
        return parsedUri[3];
    }
    return elementUri;
}
function convertMeasureLocators(locators) {
    const dataColumnLocators = locators.map((locator) => {
        if (isAttributeLocator(locator)) {
            return {
                [locator.attributeLocatorItem.attributeIdentifier]: extractItemValueFromElement(locator.attributeLocatorItem.element),
            };
        }
...
function dimensionsWithSorts(dims, sorts) {
    ...
    sorts.forEach((sortItem) => {
        if (isAttributeSort(sortItem)) {
            ...
            const attributeSortKey = {
                attribute: Object.assign(Object.assign({ attributeIdentifier }, (direction !== SortKeyAttributeAttributeDirectionEnum.ASC ? { direction } : {})), { sortType: convertAttributeSortType(sortItem) }),
            };
...
export function dimensionLocalIdentifier(idx) {
    return `dim_${idx}`;
}
/**
...
export function convertDimensions(def) {
    const dimensionsWithoutSorts = def.dimensions.map((dim, idx) => {
        return {
