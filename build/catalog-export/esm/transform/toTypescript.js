...
import { createUniqueVariableName, stringToVariableName } from "./titles.js";
//
// Constants
//
...
const FACT_AGGREGATIONS = ["sum", "avg", "min", "max", "median", "runsum"];
//
 ...
 */
function uniqueVariable(title, nameScope = GlobalNameScope) {
 ...
 */
function dateDataSetProperty(title, nameScope = GlobalNameScope) {
...
function dateDataSetAttributeProperty(title, nameScope = GlobalNameScope) {
    const datasetStart = title.lastIndexOf("(");
    if (datasetStart === -1) {
        return uniqueVariable(title, nameScope);
    }
 ...
 */
function dateAttributeSwitcharoo(title, nameScope = GlobalNameScope) {
    const datasetStart = title.lastIndexOf("(");
    if (datasetStart === -1) {
        return uniqueVariable(title, nameScope);
    }
 ...
 */
function dateDisplayFormStrip(title, nameScope = GlobalNameScope) {
...
function generateSdkModelImports() {
    return {
        moduleSpecifier: "@gooddata/sdk-model",
        namedImports: ["newAttribute", "newMeasure", "IAttribute", "IMeasure", "IMeasureDefinition", "idRef"],
        leadingTrivia: "// @ts-ignore ignore unused imports here if they happen (e.g. when there is no measure in the workspace)",
    };
}
function generateAttributeDisplayForm(displayForm, attributeVariableName, nameScope, naming) {
 ...
 */
function generateDisplayFormPropertyInitializers(attribute, attributeVariableName, naming = DefaultNaming) {
    const localNameScope = {};
    return attribute.attribute.content.displayForms.map((df) => generateAttributeDisplayForm(df, attributeVariableName, localNameScope, naming));
}
/**
 ...
 */
function generateAttributeConstant(attribute, naming = DefaultNaming) {
    ...
    if (displayForms.length === 1) {
         ...
         */
        const displayFormInits = generateDisplayFormPropertyInitializers(attribute, variableName, naming);
        return {
            declarationKind: VariableDeclarationKind.Const,
            isExported: true,
            docs,
...
function generateAttributes(projectMeta) {
    return projectMeta.catalog.attributes.map((a) => generateAttributeConstant(a));
}
function generateMeasureFromMetric(metric) {
...
function generateFactAggregations(fact) {
...
function generateMeasuresFromFacts(fact) {
    const { meta } = fact.fact;
    const variableName = uniqueVariable(meta.title);
    const aggregationInits = generateFactAggregations(fact);
    return {
        declarationKind: VariableDeclarationKind.Const,
        isExported: true,
 ...
 */
function generateMeasures(projectMeta) {
...
function generateDateDataSetAttributes(dd, naming) {
    const ddScope = {};
...
function generateDateDataSetInitialzier(dd, naming, namingScope) {
 ...
 */
function generateDateDataSetMapping(dds, naming) {
    const mappingScope = {};
...
function generateDateDataSets(projectMeta, tiger) {
 ...
 */
function generateInsights(projectMeta) {
    const insightInitializer = projectMeta.insights.map((insight) => {
 ...
 */
function generateAnalyticalDashboards(projectMeta) {
    const analyticalDashboardInitializer = projectMeta.analyticalDashboards.map((dashboard) => {
 ...
 */
export function transformToTypescript(projectMeta, outputFile, tiger) {
    GlobalNameScope = {};
