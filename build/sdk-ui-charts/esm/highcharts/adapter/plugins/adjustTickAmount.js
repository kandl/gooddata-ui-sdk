...
export function adjustTicks(axis) {
    ...
    const { tickAmount, tickInterval } = axis;
    ...
    if (currentTickAmount < tickAmount) {
        while (tickPositions.length < tickAmount) {
            const isAddFirst = dataMax <= 0 || // negative dataSet
                max <= 0 ||
...
export function alignToBaseAxis(yAxis, baseYAxis) {
    ...
    for (let direction = getDirection(baseYAxis, yAxis); direction !== ALIGNED; direction = getDirection(baseYAxis, yAxis)) {
        let tickPositions = yAxis.tickPositions.slice();
...
export function preventDataCutOff(axis) {
    ...
    axis.tickInterval *= 2;
    axis.tickPositions = axis.tickPositions.map((value) => value * 2);
...
function alignYAxes(axis) {
    ...
    const direction = getDirection(baseYAxis, alignedYAxis);
    const isReadyToAlign = axis.opposite && direction !== ALIGNED;
    const hasLineChart = isAxisWithLineChartType(baseYAxis) || isAxisWithLineChartType(alignedYAxis);
...
export function shouldBeHandledByHighcharts(axis) {
    ...
    const yAxes = getYAxes(axis.chart);
    return yAxes.some((axis) => axis.visible === false);
}
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
export const adjustTickAmount = (HighchartsInstance) => {
