...
program
    ...
    .option("--backend <backend>", "Type of backend against that you are targeting. Either GoodData Platform (bear) or GoodData.CN (tiger)");
...
const listDashboardsCmd = listCmd
    ...
    await listCmdAction(listDashboards, {
        ...
        commandOpts: Object.assign(Object.assign({}, listCmd.opts()), listDashboardsCmd.opts()),
    });
});
const listDashboardPluginsCmd = listCmd
    ...
    await listCmdAction(listDashboardPlugins, {
        programOpts: program.opts(),
        commandOpts: Object.assign(Object.assign({}, listCmd.opts()), listDashboardPluginsCmd.opts()),
    });
});
...
const inspectDashboardCmd = inspectCmd
    ...
    await inspectCmdAction(identifier, inspectDashboard, {
        programOpts: program.opts(),
        commandOpts: Object.assign(Object.assign({}, inspectCmd.opts()), inspectDashboardCmd.opts()),
    });
});
const initCmd = dashboardCmd
    .command("init")
    .description("Initialize a new dashboard plugin")
    .argument("[plugin-name]", "Name of the plugin to create. Plugin name will be used to " +
    "create a new subdirectory for the plugin project. The program expects that the subdirectory does not exist")
    .option("--language <language>", "Programming language to use for plugin. Either TypeScript (ts) or JavaScript (js)")
    .option("--workspace-id <workspace>", "Identifier of workspace that contains dashboard that you want to enhance using a plugin")
    .option("--dashboard-id <dashboard>", "Identifier of dashboard that you want to enhance using a plugin")
    .option("--target-dir <path>", "Path to the directory to create the plugin in. If not " +
...
const addPluginCmd = dashboardCmd
    .command("add")
    .description("Add plugin into a workspace so that it can be used on the dashboards in that workspace")
    .argument("<plugin-url>", "URL of location where the plugin assets are hosted. The URL must use https protocol")
    ...
    .option("--dry-run", "In dry run mode, the tool will proceed up to the point when first update operation has to " +
    "be done and then will stop. This is ideal to verify configuration. Dry run is disabled by default", false)
...
const linkPluginCmd = dashboardCmd
    .command("link")
    ...
    .option("--workspace-id <id>", "Identifier of workspace that contains dashboard and plugin that should be linked")
    .option("--dashboard-id <id>", "Identifier of dashboard to which you want to link the plugin")
    .option("--dry-run", "In dry run mode, the tool will proceed up to the point when first update operation has to " +
    "be done and then will stop. This is ideal to verify configuration. Dry run is disabled by default", false)
    .option("--with-parameters", "Indicate that the link between dashboard and plugin should be parameterized. When you specify " +
...
const unlinkPluginCmd = dashboardCmd
    .command("unlink")
    ...
    .option("--workspace-id <id>", "Identifier of workspace that contains dashboard that should use the plugin")
...
const updatePluginParamsCmd = dashboardCmd
    ...
    .option("--workspace-id <id>", "Identifier of the workspace that contains dashboard that should use the plugin.")
...
const removePluginParamsCmd = dashboardCmd
...
function acceptUntrustedSsl(options) {
