...
export function createCredentialsFromEnv(env) {
    return {
...
export async function promptCredentials(backend) {
    if (backend === "bear") {
 ...
 */
export function validateCredentialsComplete(backend, credentials) {
 ...
 * @param credentials - gathered credentials
 */
export function completeCredentialsOrDie(backend, credentials) {
    const validationError = validateCredentialsComplete(backend, credentials);
    switch (validationError) {
        case "USERNAME_MISSING":
            throw new InputValidationError("username", "", "Unable to determine username to use when logging into GoodData platform. Please make sure GDC_USERNAME env variable is set in your session or in the .env.secrets file");
        case "PASSWORD_MISSING":
            throw new InputValidationError("password", "", "Unable to determine password to use when logging into GoodData platform. Please make sure GDC_PASSWORD env variable is set in your session or in the .env.secrets file");
        case "TOKEN_MISSING":
            throw new InputValidationError("token", "", "Unable to determine token to use for authentication to GoodData.CN. Please make sure TIGER_API_TOKEN env variable is set in your session or in the .env.secrets file");
    }
}
