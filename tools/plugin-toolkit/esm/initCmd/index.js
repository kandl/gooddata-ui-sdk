...
import { convertToPluginDirectory, genericErrorReporter, readJsonSync, writeAsJsonSync, } from "../_base/utils.js";
...
//
//
function unpackProject(target, language) {
 ...
 * @param config - config for the initialization action
 */
function renamePluginDirectories(target, config) {
    fse.renameSync(path.join(target, "src", "plugin"), path.join(target, "src", config.pluginIdentifier));
    fse.renameSync(path.join(target, "src", "plugin_engine"), path.join(target, "src", `${config.pluginIdentifier}_engine`));
    fse.renameSync(path.join(target, "src", "plugin_entry"), path.join(target, "src", `${config.pluginIdentifier}_entry`));
}
function performReplacementsInFiles(dir, config) {
    ...
    const replacements = {
        ...
        "README.md": [
            ...
            {
                regex: /\{\{protocol\}\}/g,
                value: protocol !== null && protocol !== void 0 ? protocol : "https:",
            },
        ],
        scripts: {
            "refresh-md.js": [
...
function runInstall(target, config) {
    ...
    if (skipInstall) {
        logWarn(`Skipping installation of plugin project dependencies. Make sure to run '${packageManager} install' in the plugin directory before you start developing.`);
        return;
    }
    try {
        const args = ["install"];
...
export async function initCmdAction(pluginName, options) {
    try {
        logInfo("You are about to create project for a new dashboard plugin. Please note that the " +
        ...
        if (fse.existsSync(target)) {
            logError(`Directory for your new plugin already exists: ${target}. Either use a different plugin name or use --target-dir and specify a different directory.`);
            process.exit(1);
        }
        await prepareProject(target, config);
        runInstall(target, config);
        logSuccess(`A new project for your dashboard plugin is ready in: ${target}. Check out the package.json and fill in author and description if possible.`);
    }
    catch (e) {
