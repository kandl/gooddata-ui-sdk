...
import { getRemovePluginParamsCmdConfig } from "./actionConfig.js";
...
async function updateDashboardWithRemovedParams(config) {
    var _a;
    ...
    const plugins = (_a = dashboardObj.plugins) === null || _a === void 0 ? void 0 : _a.map((plugin) => {
        if (areObjRefsEqual(plugin.plugin, touchedPlugin === null || touchedPlugin === void 0 ? void 0 : touchedPlugin.ref)) {
            return Object.assign(Object.assign({}, plugin), { parameters: undefined });
        }
        return plugin;
    });
...
export async function removePluginParamCmdAction(identifier, options) {
    try {
        const config = await getRemovePluginParamsCmdConfig(identifier, options);
        ...
        if (success) {
            logSuccess(`Parameters on linked plugin ${config.identifier} on dashboard ${config.dashboard} has been removed.`);
        }
    }
    catch (e) {
