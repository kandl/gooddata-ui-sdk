...
import { getLinkCmdActionConfig } from "./actionConfig.js";
import { idRef } from "@gooddata/sdk-model";
import ora from "ora";
import { genericErrorReporter } from "../_base/utils.js";
function printUseConfigSummary(config) {
    const { backend, hostname, workspace, dashboard, identifier, parameters, withParameters, credentials: { username }, } = config;
...
async function updateDashboardWithPluginLink(config) {
...
export async function linkPluginCmdAction(identifier, options) {
    try {
        const config = await getLinkCmdActionConfig(identifier, options);
