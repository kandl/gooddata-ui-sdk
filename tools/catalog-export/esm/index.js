...
import { getConfigFromConfigFile, getConfigFromEnv, getConfigFromOptions, getConfigFromPackage, mergeConfigs, } from "./base/config.js";
import { DEFAULT_CONFIG, DEFAULT_CONFIG_FILE_NAME, DEFAULT_OUTPUT_FILE_NAME } from "./base/constants.js";
import { isCatalogExportError } from "./base/types.js";
...
program
    ...
    .option("--catalog-output <value>", `Output file (defaults to ${DEFAULT_OUTPUT_FILE_NAME}). The output file will be created in current working directory`)
    .option("--hostname <url>", `Instance of GoodData platform.`)
    .option("--config <path>", `Custom config file (default ${DEFAULT_CONFIG_FILE_NAME})`)
    .option("--backend <backend>", "Indicates type of the backend that runs on the hostname. Can be either tiger for GoodData Cloud or GoodData.CN or bear for the GoodData platform. Default: tiger")
    .option("--accept-untrusted-ssl", "Allows to run the tool with host, that has untrusted ssl certificate")
    .parse(process.argv);
async function loadProjectMetadataFromBackend(config) {
...
async function run() {
    ...
    try {
        ...
        const mergedConfig = mergeConfigs(...(await Promise.all([
            DEFAULT_CONFIG,
        ...
        logBox(chalk `The result generated from workspace with id {bold ${projectMetadata.workspaceId}} is located at {bold ${filePath}}`);
        process.exit(0);
    }
    catch (err) {
