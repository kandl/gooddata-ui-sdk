...
import { CatalogExportError, getConfiguredWorkspaceId, } from "../../base/types.js";
import ora from "ora";
import { logError, logInfo } from "../../cli/loggers.js";
import { promptWorkspaceId } from "../../cli/prompts.js";
 ...
 */
async function probeAccess(client) {
    var _a;
    try {
 ...
 * empty value.
 */
function getTigerApiToken(config) {
    const token = config.token;
    return (token === null || token === void 0 ? void 0 : token.length) ? token : undefined;
}
/**
 * Gets the tiger client asking for credentials if they are needed.
 */
async function getTigerClient(config) {
    const token = getTigerApiToken(config);
    const hasToken = token !== undefined;
    let askedForLogin = false;
    ...
    try {
        ...
        if (!hasAccess) {
            ...
            logInfo("1. You should now see your default browser open at either your Tiger developer settings page " +
                "or login page (if so, please log in).");
            logInfo("2. Once you are on your Tiger developer settings page please " +
            ...
            logInfo(`3. Once you have the token value, please set the ${API_TOKEN_VAR_NAME} environment variable and try again.`);
...
async function loadWorkspaces(client) {
...
async function selectWorkspace(client) {
    ...
    const choices = workspaces.data.map((ws) => {
        var _a, _b;
        return {
            name: (_b = (_a = ws.attributes) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : ws.id,
            value: ws.id,
        };
    });
 ...
 */
export async function loadWorkspaceMetadataFromTiger(config) {
